...
const Quize2 = () => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        const response = await axios.get("http://localhost:4002/quizRouter");
  ...
  const handleGoToTask = () => {
    window.location.href='https://tasks-management-three.vercel.app/';
  };
        ...
        <div className="sub-rect" src={bgImage3}>
          ...
          {currentQuestionIndex < quizzes.length ? (
            ...
            <>
              ...
              <button onClick={handleGoToTask}>Go to Task</button>
            </>
          )}
        </div>
      </div>
